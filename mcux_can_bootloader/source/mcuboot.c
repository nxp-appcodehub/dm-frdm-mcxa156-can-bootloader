...
static void handle_cmd(mcuboot_t *ctx, kptl_t *pkt)
{
    packet_ack_t ack;
    cmd_hdr_t rx_cp;
    uint32_t tx_param[7];
    ...
    switch(rx_cp.tag)
    {
        case kCommandTag_GetProperty:
            ...
            ctx->op_send((uint8_t*)&ctx->tx_pkt, kptl_get_frame_size(&ctx->tx_pkt));
            break;
